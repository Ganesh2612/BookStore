 ...
 cartItems: any[] = [];
  ...
  fetchCartItems(): void {
    this.bookService.getCart().subscribe({
      ...
      error: (err:any) => console.error('Failed to fetch cart items:', err),
    });
  }
  loadUserInfo(): void {
  ...
  updateCart(item: any): void {
    this.bookService.updateCart(item._id, item.quantityToBuy).subscribe({
      next: () => {},
      error: (err:any) => console.error('Failed to update cart quantity:', err),
    });
  }
  ...
  removeFromCart(cartItemId: string): void {
    this.bookService.removeCart(cartItemId).subscribe({
      ...
      error: (err:any) => console.error('Failed to remove from cart:', err),
    });
  }
  ...
  orderNow(): void {
    ...
    this.bookService.addOrder(orderPayload).subscribe({
      ...
      error: (err:any) => console.error('Failed to place order:', err),
    });
  }
}
